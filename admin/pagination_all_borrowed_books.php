...
if (!empty($search_term)) {
    ...
    $where_clause_search = "UPPER(book_list.title) LIKE UPPER(?)
                        ...
                        OR UPPER(book_list.email) LIKE UPPER(?)
                        OR UPPER(book_list.card_no) LIKE UPPER(?)";
    for ($i = 0; $i < 5; $i++) {
...
$book_list_subquery = "
    SELECT books.id AS book_id, books.title, books.isbn, 
        GROUP_CONCAT(CONCAT(authors.first_name, ' ', authors.last_name) SEPARATOR ', ') AS authors, borrowed_books.user_id, borrowed_books.borrowed_at, borrowed_books.due_date, users.email, users.card_no
    FROM borrowed_books
...
include __DIR__ . "/../fragmente/list_all_borrowed_books.php";
